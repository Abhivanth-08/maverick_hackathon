...
from fastapi.responses import HTMLResponse
from backend.app.ai.graph_rag import generate_graph_rag_reasoning
from backend.app.graph_builder import generate_patient_graph_html

@app.get("/api/patients/{patient_id}/graph_rag/{match_id}")
def get_graph_rag(patient_id: int, match_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    reasoning = generate_graph_rag_reasoning(db, match_id)
    return {"match_id": match_id, "patient_id": patient_id, "reasoning": reasoning}

@app.get("/api/patients/{patient_id}/graph", response_class=HTMLResponse)
def get_patient_graph(patient_id: int, db: Session = Depends(get_db)):
    return HTMLResponse(content=generate_patient_graph_html(db, patient_id))
